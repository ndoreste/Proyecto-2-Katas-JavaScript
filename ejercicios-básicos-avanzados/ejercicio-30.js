...
//  Objeto vacío para almacenar canciones por género
const songsByGenre = {};

// Recorrer la lista de canciones con un bucle for
for (let i = 0; i < tracks.length; i++) {
  const track = tracks[i];
  const genre = track.genre;

  // Si el género no existe en el objeto, crear un array vacío
  if (!songsByGenre[genre]) {
    songsByGenre[genre] = [];
  }

  // Agregar la canción al array correspondiente del género
